...
export const policeService = {
  async getDashboard() {
    const [
      ...
      () => prisma.investigation.groupBy({ by: ["city"], _count: { _all: true }, where: { city: { not: null } } }),
      ...
      () => prisma.threatScan.groupBy({ by: ["scanType"], _count: { _all: true }, orderBy: { _count: { id: "desc" } } }),
    ...
    reportStats.forEach((r) => { reportStatsMap[r.status.toLowerCase()] = (r._count as { _all: number })._all ?? 0; });
    ...
    return {
      ...
      cityBreakdown: threatsByCity.map((c) => ({ city: c.city, count: (c._count as { _all: number })._all ?? 0 })),
      ...
      topThreatCategories: topThreats.map((t) => ({ type: t.scanType.toLowerCase(), count: (t._count as { _all: number })._all ?? 0 })),
