...
export const evidenceService = {
  async upload(input: UploadInput) {
    ...
    const existing = await prisma.evidenceUpload.findUnique({
      ...
      include: { threat_reports: { select: { id: true, reportNumber: true, type: true } } },
    });

    if (existing) {
      let currentReport = existing.threat_reports;
      ...
      if (input.reportId && existing.reportId !== input.reportId) {
        ...
        currentReport = updated.threat_reports;
      }
  ...
  async list(userId: string, page = 1, limit = 20) {
    ...
    return {
      items: items.map((e) => ({
        ...
        reportNumber: e.threat_reports?.reportNumber || null,
        reportType: e.threat_reports?.type || null,
  ...
  async listAllPolice(params: { status?: string; riskLevel?: string; page?: number; limit?: number }) {
    ...
    const [items, total] = await Promise.all([
      prisma.evidenceUpload.findMany({
        ...
        include: {
          ...
          threat_reports: { select: { id: true, reportNumber: true, type: true, description: true, status: true } },
        },
      }),
    ...
    return {
      items: items.map((e) => ({
        ...
        reportNumber: e.threat_reports?.reportNumber || null,
        reportType: e.threat_reports?.type || null,
        reportDescription: e.threat_reports?.description?.slice(0, 80) || null,
        reportStatus: e.threat_reports?.status?.toLowerCase() || null,
        createdAt: e.createdAt.toISOString(),
        storagePath: e.storagePath,
      })),
  ...
  async policeGetById(id: string) {
    const e = await prisma.evidenceUpload.findUnique({
      ...
      include: {
        user: { include: { profile: true } },
        threat_reports: { select: { id: true, reportNumber: true, type: true, description: true, status: true, priority: true, scammerContact: true, financialLoss: true, createdAt: true } },
      },
    });
    if (!e) return null;
    ...
    return {
      ...
      reportNumber: e.threat_reports?.reportNumber || null,
      reportType: e.threat_reports?.type || null,
      reportDescription: e.threat_reports?.description || null,
      reportStatus: e.threat_reports?.status?.toLowerCase() || null,
      reportPriority: e.threat_reports?.priority?.toLowerCase() || null,
      reportScammerContact: e.threat_reports?.scammerContact || null,
      reportFinancialLoss: e.threat_reports?.financialLoss || null,
      reportCreatedAt: e.threat_reports?.createdAt?.toISOString() || null,
      createdAt: e.createdAt.toISOString(),
      storagePath: e.storagePath,
    };
  },
